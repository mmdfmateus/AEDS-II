...
public class ArvoreB {
  ...
  private Item pesquisa (Item reg, Pagina ap, boolean comp) { //O boolean quando true conta o número de páginas visitadas e guarda em pageVisits
    if (ap == null) return null; // Registro @{\it n\~ao}@ econtrado
      ...
      if (reg.compara (ap.r[i]) == 0){
          ...
          if(comp)
              comparacoes += 2;
          pageVisits++;
          return pesquisa(reg, ap.p[i], comp);
      }
      else {
          if(comp)
              comparacoes += 2;
          pageVisits++;
          return pesquisa(reg, ap.p[i+1], comp);
      }
    }
  }
  ...
  public boolean insere (Item reg) {
    comparacoes = 0;
